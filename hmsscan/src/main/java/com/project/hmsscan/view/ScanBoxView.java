...
import com.project.hmsscan.util.BitmapUtil;
...
public class ScanBoxView extends View {
    ...
    private final int MAX_BOX_SIZE = ScreenUtil.dp2px(getContext(), 300);
    private final int SCAN_LINE_HEIGHT = ScreenUtil.dp2px(getContext(), 3.0f);

    private final int mCornerRound = ScreenUtil.dp2px(getContext(), 3.0f);
    private final int mCornerMargin = ScreenUtil.dp2px(getContext(), 9.0f);
    ...
    private void init() {
        ...
        mMaskColor = resources.getColor(R.color.hms_scan_mask);
        mTextColor = resources.getColor(R.color.hms_text_normal);
        mTextColorBig = resources.getColor(R.color.hms_text_big);

        mScanLineColor1 = resources.getColor(R.color.hms_scan_1);
        mScanLineColor2 = resources.getColor(R.color.hms_scan_2);
        mScanLineColor3 = resources.getColor(R.color.hms_scan_3);

        mTopOffset = -ScreenUtil.dp2px(context, 10);
        mBoxSizeOffset = ScreenUtil.dp2px(context, 40);
        mBoxSize = ScreenUtil.dp2px(context, 200);

        mBorderColor = resources.getColor(R.color.hms_scan_border);
        mBorderStrokeWidth = ScreenUtil.dp2px(context, 0.5f);

        mCornerColor = resources.getColor(R.color.hms_scan_corner);
        mCornerLength = ScreenUtil.dp2px(context, 17);
        mCornerSize = ScreenUtil.dp2px(context, 3);
        mHalfCornerSize = 1.0f * mCornerSize / 2;

        mTextSize = ScreenUtil.sp2px(context, 12);
        mTextSizeBig = ScreenUtil.sp2px(context, 16);
        mTxtPaint.setTextSize(mTextSize);
        mTxtPaint.setTextAlign(Paint.Align.CENTER);
        mTxtPaint.setColor(Color.GRAY);
        mTxtPaint.setStyle(Paint.Style.FILL);

        mFlashLightOnText = getResources().getText(R.string.hms_click_open_flash_light).toString();
        mFlashLightOffText = getResources().getText(R.string.hms_click_close_flash_light).toString();
        mScanNoticeText = getResources().getText(R.string.hms_scan_notice).toString();
    }
...
//                mTxtPaint);
    ...
    private void moveScanLine() {
        ...
        if (mScanlineOrientation == VERTICAL) {
            mScanLineAnimator = ValueAnimator.ofFloat(0, mBoxHeight - mBorderStrokeWidth * 5);
...
//                            ((int) (mBoxTop + mScanLinePosition + SCAN_LINE_HEIGHT + 10)));
            ...
            mScanLineAnimator = ValueAnimator.ofFloat(0, mBoxWidth - mBorderStrokeWidth * 5);
            mScanLineAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
                @Override
                public void onAnimationUpdate(ValueAnimator animation) {
                    mScanLinePosition = (float) animation.getAnimatedValue();
